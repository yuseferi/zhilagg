...
/**
 * View builder handler for zhilagg feeds.
 */
class FeedViewBuilder extends EntityViewBuilder {
  ...
  public static function createInstance(ContainerInterface $container, EntityTypeInterface $entity_type) {
    return new static(
      ...
      $container->get('config.factory')->get('zhilagg.settings')
    );
  }
    ...
    foreach ($entities as $id => $entity) {
      ...
      if ($display->getComponent('more_link')) {
        ...
        $build[$id]['more_link'] = array(
          ...
          '#url' => Url::fromRoute('entity.zhilagg_feed.canonical', ['zhilagg_feed' => $entity->id()]),
          '#options' => array(
            'attributes' => array(
              'title' => $title_stripped,
            ),
