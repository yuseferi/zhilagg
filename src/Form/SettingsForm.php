...
/**
 * Configures zhilagg settings for this site.
 */
class SettingsForm extends ConfigFormBase {

  /**
   * The zhilagg plugin managers.
   *
  ...
  /**
   * The zhilagg plugin definitions.
   *
   * @var array
   */
  ...
  /**
   * Constructs a \Drupal\zhilagg\SettingsForm object.
   *
   ...
   */
  public function __construct(ConfigFactoryInterface $config_factory, ZhilaggPluginManager $fetcher_manager, ZhilaggPluginManager $parser_manager, ZhilaggPluginManager $processor_manager, TranslationInterface $string_translation) {
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('plugin.manager.zhilagg.fetcher'),
      $container->get('plugin.manager.zhilagg.parser'),
      $container->get('plugin.manager.zhilagg.processor'),
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // Global zhilagg settings.
    $form['zhilagg_allowed_html_tags'] = array(
      '#type' => 'textfield',
    ...
    if (count($this->definitions['fetcher']) > 1) {
      $basic_conf['zhilagg_fetcher'] = array(
        '#type' => 'radios',
    ...
    if (count($this->definitions['parser']) > 1) {
      $basic_conf['zhilagg_parser'] = array(
        '#type' => 'radios',
        '#title' => $this->t('Parser'),
    ...
    if (count($this->definitions['processor']) > 1) {
      $basic_conf['zhilagg_processors'] = array(
        '#type' => 'checkboxes',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config->set('items.allowed_html', $form_state->getValue('zhilagg_allowed_html_tags'));
    if ($form_state->hasValue('zhilagg_fetcher')) {
      $config->set('fetcher', $form_state->getValue('zhilagg_fetcher'));
    }
    if ($form_state->hasValue('zhilagg_parser')) {
      $config->set('parser', $form_state->getValue('zhilagg_parser'));
    }
    if ($form_state->hasValue('zhilagg_processors')) {
      $config->set('processors', array_filter($form_state->getValue('zhilagg_processors')));
    }
    $config->save();
  }
