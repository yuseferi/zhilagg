 ...
 * @group zhilagg
 */
class DeleteFeedTest extends ZhilaggTestBase {
  ...
  public function testDeleteFeed() {
    ...
    $block = $this->drupalPlaceBlock('zhilagg_feed_block');
    $block->getPlugin()->setConfigurationValue('feed', $feed1->id());
    $block->save();
    $block2 = $this->drupalPlaceBlock('zhilagg_feed_block');
    ...
    // Check feed source.
    $this->drupalGet('zhilagg/sources/' . $feed1->id());
    $this->assertResponse(404, 'Deleted feed source does not exists.');

    // Check database for feed.
    $result = db_query("SELECT COUNT(*) FROM {zhilagg_feed} WHERE title = :title AND url = :url", array(':title' => $feed1->label(), ':url' => $feed1->getUrl()))->fetchField();
