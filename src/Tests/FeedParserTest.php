 ...
 * @group zhilagg
 */
class FeedParserTest extends ZhilaggTestBase {
  ...
  protected function setUp() {
    parent::setUp();
    // Do not delete old zhilagg items during these tests, since our sample
    // feeds have hardcoded dates in them (which may be expired when this test
    // is run).
    $this->config('zhilagg.settings')->set('items.expire', AGGREGATOR_CLEAR_NEVER)->save();
  }
  ...
  public function testAtomSample() {
    ...
    $this->assertEqual('urn:uuid:1225c695-cfb8-4ebb-aaaa-80da344efa6a', db_query('SELECT guid FROM {zhilagg_item} WHERE link = :link', array(':link' => 'http://example.org/2003/12/13/atom03'))->fetchField(), 'Atom entry id element is parsed correctly.');
    ...
    $db_guid = db_query('SELECT guid FROM {zhilagg_item} WHERE link = :link', array(
  ...
  public function testRedirectFeed() {
    $redirect_url = Url::fromRoute('zhilagg_test.redirect')->setAbsolute()->toString();
    $feed = Feed::create(array('url' => $redirect_url, 'title' => $this->randomMachineName()));
    $feed->save();
    $feed->refreshItems();

    // Make sure that the feed URL was updated correctly.
    $this->assertEqual($feed->getUrl(), \Drupal::url('zhilagg_test.feed', array(), array('absolute' => TRUE)));
  }
