 ...
 * @group zhilagg
 */
class UpdateFeedItemTest extends ZhilaggTestBase {
  /**
   * Tests running "update items" from 'admin/config/services/zhilagg' page.
   */
  public function testUpdateFeedItem() {
    ...
    $fid = db_query("SELECT fid FROM {zhilagg_feed} WHERE url = :url", array(':url' => $edit['url[0][value]']))->fetchField();
    $feed = Feed::load($fid);

    $feed->refreshItems();
    $before = db_query('SELECT timestamp FROM {zhilagg_item} WHERE fid = :fid', array(':fid' => $feed->id()))->fetchField();
    ...
    $after = db_query('SELECT timestamp FROM {zhilagg_item} WHERE fid = :fid', array(':fid' => $feed->id()))->fetchField();
