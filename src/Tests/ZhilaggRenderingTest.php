...
/**
 * Tests display of zhilagg items on the page.
 *
 * @group zhilagg
 */
class ZhilaggRenderingTest extends ZhilaggTestBase {
  ...
  public function testBlockLinks() {
    ...
    $block = $this->drupalPlaceBlock("zhilagg_feed_block", array('label' => 'feed-' . $feed->label()));
    ...
    $items = $this->container->get('entity.manager')->getStorage('zhilagg_item')->loadByFeed($feed->id(), 1);
    ...
    $this->assertTrue(in_array('zhilagg_feed:' . $feed->id(), $cache_tags));
    ...
    $this->assertFalse(empty($correct_titles), 'Zhilagg feed page is available and has the correct title.');
    $cache_tags = explode(' ', $this->drupalGetHeader('X-Drupal-Cache-Tags'));
    $this->assertTrue(in_array('zhilagg_feed:' . $feed->id(), $cache_tags));
    $this->assertTrue(in_array('zhilagg_feed_view', $cache_tags));
    $this->assertTrue(in_array('zhilagg_item_view', $cache_tags));
  ...
  public function testFeedPage() {
    ...
    // Check the rss zhilagg page as anonymous user.
    $this->drupalLogout();
    $this->drupalGet('zhilagg/rss');
    $this->assertResponse(403);

    // Check the rss zhilagg page as admin.
    $this->drupalLogin($this->adminUser);
    $this->drupalGet('zhilagg/rss');
    $this->assertResponse(200);
    $this->assertEqual($this->drupalGetHeader('Content-type'), 'application/rss+xml; charset=utf-8');

    // Check the opml zhilagg page.
    $this->drupalGet('zhilagg/opml');
